...
//
// Worker Google operations switch (_shared/worker-google.ts): an automated
// caller (x-cron-secret — the Foundation worker) gets every op except the
// read-only gbp_locate refused with status "skipped" unless
// app_settings.worker_google_ops is { enabled: true }. The check runs before
// the Google token is fetched, so holding a valid token grants the worker no
...
import { isGoogleWrite, workerGoogleOpsEnabled, workerGoogleRefusal } from "../_shared/worker-google.ts";
...
export function createGoogleOps(deps: { supabase: Supabase; fetch: Fetch }) {
  const { supabase } = deps;
  ...
  // Worker Google operations switch: before any Google call or CRM write.
  if (isCron && isGoogleWrite(op) && !(await workerGoogleOpsEnabled(supabase))) {
    return Response.json(workerGoogleRefusal(op), { status: 403 });
  }
