  ...
  return <section className="space-y-4" aria-label="Client marketing scorecard">
    ...
    <div className="grid gap-3">
      {REPORT_AREAS.map((area, index) => {
        ...
        return <details key={area.key} className="min-w-0 rounded-lg border p-4" open={measured > 0}>
          <summary className="cursor-pointer"><span className="font-medium">{index + 1}. {area.label}</span><span className="ml-3 text-xs text-muted-foreground">{measured ? `${measured} measured` : "No verified measurements yet"}</span></summary>
