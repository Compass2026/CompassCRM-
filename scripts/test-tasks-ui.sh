...
"$PG_BIN/initdb" -D "$WORK/data" -U supabase_admin --auth=trust >/dev/null
"$PG_BIN/pg_ctl" -D "$WORK/data" -o "-c listen_addresses='' -k $WORK -p 54329" -l "$WORK/log" -w start >/dev/null
...
psql_as postgres -d sandbox -f "$ROOT/supabase/tests/sandbox/tasks_ui_fixtures.sql" -o /dev/null
# PostgREST connects as authenticator and switches to the JWT's role.
psql_as supabase_admin -d sandbox -c "create role authenticator login noinherit; grant anon, authenticated, service_role to authenticator"
...
JWT_SECRET="$JWT_SECRET" PGRST_URL="http://127.0.0.1:$PGRST_PORT" \
  ...
  node --no-warnings tests/tasks-ui.mjs
